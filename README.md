...
**Repository:** `https://github.com/Yvaine-Xyan/LinkRSP.git` · **Web UI:** https://github.com/Yvaine-Xyan/LinkRSP
...
Canonical documents: [`docs/whitepaper/linkrsp-whitepaper-v1.0-alpha.md`](docs/whitepaper/linkrsp-whitepaper-v1.0-alpha.md), [`docs/spec/linkrsp-core-algorithm-spec-v1.0.md`](docs/spec/linkrsp-core-algorithm-spec-v1.0.md), [`docs/reports/feasibility-assessment-v1.0-alpha.md`](docs/reports/feasibility-assessment-v1.0-alpha.md), full index [`docs/README.md`](docs/README.md).
...
| `D_base` | Baseline difficulty; **default 1.0**, **configurable** in later frozen `spec` |
...
Licensed under **MIT** ([`LICENSE`](LICENSE)). Contributions are welcome via PR; scope includes **HDGP RuleEngine extensions for LinkRSP** (see feasibility §6: **28** enumerated scenarios today; **30–50** engine rules including splits), red-team reports, **Handshake v2** (BLE / QR UX), and translations.
...
Stack is **not frozen**. Directional defaults and anti–technical-debt constraints are in [`docs/engineering/technology-strategy-v1.0.md`](docs/engineering/technology-strategy-v1.0.md). Summary:

| Layer | Direction | Notes |
|-------|-----------|--------|
| Policy / HDGP | Versioned **Rule Engine** via HTTP/gRPC **adapter** | Deterministic rules first; HDGP-Core remains Meta-first; runnable Engine is deployer-specific |
...
- [ ] RuleEngine LinkRSP extensions to **30–50** rules (baseline: **28** in feasibility §6)
...
**仓库克隆地址：** `https://github.com/Yvaine-Xyan/LinkRSP.git` · **网页入口：** https://github.com/Yvaine-Xyan/LinkRSP
...
权威文档入口：[`docs/whitepaper/linkrsp-whitepaper-v1.0-alpha.md`](docs/whitepaper/linkrsp-whitepaper-v1.0-alpha.md)、[`docs/spec/linkrsp-core-algorithm-spec-v1.0.md`](docs/spec/linkrsp-core-algorithm-spec-v1.0.md)、[`docs/reports/feasibility-assessment-v1.0-alpha.md`](docs/reports/feasibility-assessment-v1.0-alpha.md)，完整索引见 [`docs/README.md`](docs/README.md)。
...
| `D_base` | 基准难度，**默认 1.0**，在后续冻结 `spec` 中**可配置** |
...
以 **MIT** 开源（[`LICENSE`](LICENSE)）。欢迎 PR，范围包括 **HDGP RuleEngine 的 LinkRSP 场景扩展**（可行性 §6 当前枚举 **28** 条；引擎目标 **30–50** 条含拆分）、红队与压测报告、**Handshake v2**（BLE / 二维码 UX）、多语言文档等。
...
技术栈**未最终冻结**。方向性默认与防技术债约束见 [`docs/engineering/technology-strategy-v1.0.md`](docs/engineering/technology-strategy-v1.0.md)。摘要：

| 层 | 方向 | 说明 |
|----|------|------|
| 策略 / HDGP | **Rule Engine** 经 HTTP/gRPC **适配器**接入、策略包版本化 | 规则确定性优先；HDGP-Core 以 Meta 为主；可运行 Engine 由部署方选择 |
...
- [ ] RuleEngine LinkRSP 专属扩展至 **30–50** 条（基线 **28** 条见可行性 §6）
- [ ] Handshake v2 实现
- [ ] LRS Ledger 原型
