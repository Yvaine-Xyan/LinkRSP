...
- **回放条目的实现绑定**：当前已补出最小 abuse case 清单，但多数条目尚未绑定到自动化测试、演练脚本或上线前检查表。

换言之，当前状态是：**已有范围设计，也已有少量实现承接，并已补出最小 STRIDE、数据流 / 信任边界、攻击面清单、缓解映射与红队回放清单草案，但尚未形成完整、可持续更新的威胁模型工件。**
...
3. 为高风险入口补责任人与评审节奏，至少覆盖内部接口、账本写入、对象存储、未来语义队列。
