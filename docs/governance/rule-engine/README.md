...
为避免同一规则维护两份（例如 pre_publication 与 ipo_scan 同时需要扫描），可选使用：

- `also_scan_at`：补充触发点列表（例如 `["ipo_scan"]`）。主触发点仍以 `trigger` 为准；实现侧可在对应阶段额外触发同一规则。
...
## S-009 拆分说明（IPO 材料不一致）

`S-009` 建议拆分为：

- `S-009a`：**叙述性不一致**（LLM 审计，`ipo_scan`；不依赖外部数据）
- `S-009b`：**数据与存证不一致**（规则引擎 + 账本/索引查询，`ipo_scan`；混合类，依赖接口）

在 LRS Ledger 查询接口未就绪前，应仅运行 `S-009a`，并将 `S-009b` 视为占位规则。
