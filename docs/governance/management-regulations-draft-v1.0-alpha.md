...
**延伸**：1+1 非法态、IPO/降级导致的拓扑协调、成员资格边建模与时间轴，见 [`membership-credit-lifecycle-supplement-v0.1.md`](membership-credit-lifecycle-supplement-v0.1.md)。

---

## 二、社区生命周期与「升 S」机制（Community Lifecycle & IPO）

linkrsp 不预设社区实质，也不定义社区的内部 SOP。系统只提供 **可审计的通路与最小约束**（例如：Judge 公示/评审通道、hdgp 合规扫描与红线熔断），以保证协议履行可回放、可追溯；社区的成立、日常流程与自治治理由社区自行决定。
...
**S 级阶段（运行期）**：获「升 S」认证后，社区可发布具有刚性对价的劳务任务；积分为「硬积分」：语义上仍为**权益与优先权**，非货币化承诺；在规则允许的范围内可具有更强的调度/优先级含义，但**跨社区的可用性与可比性**由规则 scope 与披露约束，**非**「积分余额全局通兑」。（详见 [`membership-credit-lifecycle-supplement-v0.1.md`](membership-credit-lifecycle-supplement-v0.1.md)。）

**延伸**：双 S 冲突处置窗口、默认策略、成员资格时间轴与「身份切换冷却期」的并列关系，见同目录补充稿。
...
**延伸**：账本只追加、换社区不删改历史分录、「投影视图 / scope」与非余额搬家语义，见 [`membership-credit-lifecycle-supplement-v0.1.md`](membership-credit-lifecycle-supplement-v0.1.md)。
...
**审计追溯权**：linkrsp 保留对存证记录与规则执行结果的底层审计追溯权；在严重合规风险（触发 hdgp 红线）下，系统只对**协议层通路**执行最小干预（阻断/降级/熔断并留痕），不介入社区内部治理细则。（须配套申诉与复核事件类型，见工程架构草案。）
